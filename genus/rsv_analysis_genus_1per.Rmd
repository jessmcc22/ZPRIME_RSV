...
usepalette <- c(usepalette[which(usepalette != "#848482")])

usepalette[9] <- "#ba75bf"
usepalette[11] <- "#61c2ae"
usepalette[16] <-"#9E9E9EFF"

usepalette <- (rev(usepalette[1:16]))
...
rankedAbundanceSample <- psmelt(zp_rel_abund) %>%
    group_by(Sample, OTU) %>%
    summarise(rel_abund = mean(Abundance)) %>%
    filter(OTU == "Streptococcus" | OTU == "Haemophilus" | OTU == "Moraxella" | OTU == "Klebsiella") %>% #|| OTU == "Corynebacterium" | OTU == "Staphylococcus")
    group_by(Sample) %>%
    summarise(totals = sum(rel_abund)) %>%
    arrange(desc(totals))

level_for_samples <- rankedAbundanceSample$Sample
...
colnames(zp_rel_abund_df)[16] <- "Genus"

zp_rel_abund_df$Genus <- factor(zp_rel_abund_df$Genus, levels = level_for_factor)

zp_rel_abund_df$Sample <- factor(zp_rel_abund_df$Sample, levels = level_for_samples)
...
ggplot(zp_rel_abund_df, aes(color = Genus, fill = Genus, y=Abundance, x=Sample)) +
  ...
  scale_color_manual(values = usepalette) +
    ggtitle("Microbial Composition of RSV- and RSV+ Decedents")

ggsave("Plots/1pct/relativeAbundanceOrderedByTop4.pdf", dpi = 1200)
```
