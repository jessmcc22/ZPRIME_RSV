...
```
#### HeatMap 
Visualize the Relative Abundance ordered by Bray-curtis dissimilarity (as two samples share fewer taxa, the number increases 0 for exact same composition, 1 for no shared genera) results.
...
print(p)
ggsave("Plots/point1/heatmap.jpeg", dpi = 600)
...
rsv_pos_samples <- paste0("S", sample_data(zp_pos)$subject)
rsv_neg_samples <- paste0("S", sample_data(zp_neg)$subject)
    
ComplexHeatmap::Heatmap(zp_scale_bray, cluster_rows = FALSE, column_names_rot = 45)
heatmap(zp_scale_bray)
```
...
zp_umap <- umap(t(otu_table(zp_rel_abund)))
zp_labels <- row.names(tax_table(zp_rel_abund))
zp_umap_coordinates <- data.frame(zp_umap$layout)
colnames(zp_umap_coordinates) <- c("UMAP1","UMAP2")
zp_umap_coordinates$state  <- sample_data(zp_rel_abund)$state
zp_umap_coordinates$subject <- sample_data(zp_rel_abund)$subject


### add filter, e.g. all rows all zeros!
pcs <- data.frame(prcomp(t(otu_table(zp_rel_abund)))$x[,1:20])
umapData <- umap(pcs)
umaps <-data.frame(umap(pcs)$layout)
colnames(umaps) <- c("UMAP1","UMAP2")
```

#### PCA and Umap plots
```{r}
pcs$state <- umaps$state <- sample_data(zp_rel_abund)$state
pcs$subject <- umaps$subject <- sample_data(zp_rel_abund)$subject

pcs1 <- ggplot(pcs, aes(x = PC1, y = PC2, color = state)) +
 geom_point(size = 0.5) +
 xlab("PC 1") +
 ylab("PC 2") +
 theme(plot.title = element_text(hjust = 0.5)) +
 ggtitle("PCA Plot (Color = State)")

pcs2 <- ggplot(pcs, aes(x = PC1, y = PC2, color = subject)) +
 geom_point(size = 0.5) +
 xlab("PC 1") +
 ylab("PC 2") +
 theme(plot.title = element_text(hjust = 0.5)) +
 ggtitle("PCA Plot (Color = subject)")

umaps1 <- ggplot(zp_umap_coordinates, aes(x = UMAP1, y = UMAP2, color = state)) +
 geom_point(size = 0.5) +
 xlab("V1") +
 ylab("V2") +
 theme(plot.title = element_text(hjust = 0.5)) +
 ggtitle("UMAP Plot (Color = State)")

umaps1

umaps2 <- ggplot(zp_umap_coordinates, aes(x = UMAP1, y = UMAP2, color = subject)) +
 geom_point(size = 0.5) +
 xlab("V1") +
 ylab("V2") +
 theme(plot.title = element_text(hjust = 0.5)) +
 ggtitle("UMAP Plot (Color = subject)")

umaps2
# umapFig <- ggarrange(pcs1, pcs2, umaps1, umaps2,
#                               ncol = 2, labels = c("A", "B", "C", "D")) +
#     bgcolor("white")
# umapFig

#grid.arrange(pcs1, pcs2, umaps1, umaps2, nrow=2)
```
