...
plot_bar(zp_rel_abund, fill = "genus") +
  ...
  scale_fill_paletteer_d(chosen_pal)+
  scale_color_paletteer_d(chosen_pal)
#ggsave(paste0("Plots/1pct/paletteoptions/", color, ".jpeg"))
...
ggsave("Plots/1pct/meanRelativeAbundance.jpeg")
```
...
ggsave("Plots/1pct/shannonAndSimpson_state.jpeg")
```
...
ggsave("Plots/1pct/t_test_sig_state.jpeg", dpi = 300)
```
...
ggsave("Plots/1pct/wilcox_sig_state.jpeg", dpi = 300, width = 5, height = 4)  
}else{
...
ggsave("Plots/1pct/shannonAndSimpson_location.jpeg")
```
...
ggsave("Plots/1pct/t_test_sig_location.jpeg")
```
## Wilcoxon rank-sum
```{r, message=FALSE, warning=FALSE}
...
if(length(significant_taxa$Genus) != 0){
  ...
  theme(legend.position = "none")
  locationFig
  ggsave("Plots/1pct/wilcox_sig_location.jpeg", dpi = 300)  
}else{
...
if(length(row.names(significant_taxa)) != 0){
    ...
    ggsave("Plots/1pct/t_test_sig_mother_hiv.jpeg")
}else{
...
if(length(significant_taxa$Genus) != 0){
  ...
  ggsave("Plots/1pct/wilcox_sig_HIV_Exposure.jpeg", dpi = 300)  
}else{
    HIV_statusFig <- psmelt(zp_rel_abund) %>%
  ggplot(data = ., aes(x = mother_hiv, y = Abundance)) +
  geom_boxplot(outlier.shape  = NA) +
...
ggsave("Plots/1pct/kruskal_sig_hospital_duration.jpeg")
}else{
    psmelt(zp_rel_abund) %>%
    ggplot(data = ., aes(x = as.factor(hosp_duration), y = Abundance)) +
    geom_boxplot(outlier.shape  = NA) +
...
ggsave("Plots/1pct/shannonAndSimpson_age.jpeg")
```
