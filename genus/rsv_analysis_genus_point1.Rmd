...
ggsave("Plots/point1/meanRelativeAbundance.jpeg")
```
...
ggsave("Plots/point1/shannonAndSimpson_state.jpeg")
```
...
for(genus in colnames(zp_pos_otu)){
    t_test[[genus]] <- round(wilcox.test(zp_pos_otu[[genus]], zp_neg_otu[[genus]], paired = TRUE)$p.value, 4)
}
...
ggsave("Plots/point1/t_test_sig_state.jpeg", dpi = 300)
```

### Wilcoxon rank-sum test
This is a  non-parametric Wilcoxon rank-sum test where the null hypothesis is equal medians between states.
```{r, message=FALSE, warning=FALSE}
...
ggsave("Plots/point1/wilcox_sig_state.jpeg", dpi = 300, width = 5, height = 4)  
}else{
...
ggsave("Plots/point1/shannonAndSimpson_location.jpeg")
```
...
## T-test for significant difference between location for any specific genus
Perform two sample t-test to determine if any genus differs significantly
...
ggsave("Plots/point1/t_test_sig_location.jpeg")
```
## Wilcoxon rank-sum
```{r, message=FALSE, warning=FALSE}
...
if(length(significant_taxa$Genus) != 0){
  ...
  theme(legend.position = "none")
  locationFig
  ggsave("Plots/point1/wilcox_sig_location.jpeg", dpi = 300)  
}else{
...
if(length(row.names(significant_taxa)) != 0){
    ...
    ggsave("Plots/point1/t_test_sig_mother_hiv.jpeg")
}else{
...
if(length(significant_taxa$Genus) != 0){
  ...
  ggsave("Plots/point1/wilcox_sig_HIV_Exposure.jpeg", dpi = 300)  
}else{
    HIV_statusFig <- psmelt(zp_rel_abund) %>%
  ggplot(data = ., aes(x = mother_hiv, y = Abundance)) +
  geom_boxplot(outlier.shape  = NA) +
...
ggsave("Plots/point1/kruskal_sig_hospital_duration.jpeg")
}else{
    psmelt(zp_rel_abund) %>%
    ggplot(data = ., aes(x = as.factor(hosp_duration), y = Abundance)) +
    geom_boxplot(outlier.shape  = NA) +
...
ggsave("Plots/point1/shannonAndSimpson_age.jpeg")
```
...
#Clean p-value chart
```{r, message=FALSE, warning=FALSE}
t_test <- dplyr::select(t_test, -len_hosp_stay, -BH_FDR_len_hosp_stay) 
